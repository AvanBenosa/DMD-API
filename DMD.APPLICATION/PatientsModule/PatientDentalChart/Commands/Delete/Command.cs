...
namespace DMD.APPLICATION.PatientsModule.PatientDentalChart.Commands.Delete
{
    [JsonSchema("DeletePatientDentalChartCommand")]
    ...
    public class CommandHandler : IRequestHandler<Command, Response>
    {
        ...
        public async Task<Response> Handle(Command request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var surfaces = await dbContext.PatientTeethSurface
                    .Where(x => x.PatientTeethId == item.Id)
                    .ToListAsync(cancellationToken);
                var images = await dbContext.PatientTeethImages
                    .Where(x => x.PatientTeethId == item.Id)
                    .ToListAsync(cancellationToken);

                if (surfaces.Count > 0)
                {
                    dbContext.PatientTeethSurface.RemoveRange(surfaces);
                }

                if (images.Count > 0)
                {
                    dbContext.PatientTeethImages.RemoveRange(images);
                }

                dbContext.PatientTeeth.Remove(item);
